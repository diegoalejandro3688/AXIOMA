...
**Estado**: Definición revisada (4ª pasada, 2026-08-06). Incremento 1 ("Fundación de temporadas y ligas") **implementado y gateado** (§9). Incremento 2 ("Ranking") **implementado, gateado y CERRADO** (§10, ADR-0020 APPROVED) — checkpoint commiteado sin tag (`26c28f1`). La limitación ambiental de `findPendingGrant` (Bloque I) documentada al cierre de Incremento 2 (§10, nota final) fue diagnosticada y corregida en un commit independiente (`0b52122`, starvation real confirmada y resuelta) — ver también `449d654`/`15fcd30` (hallazgo relacionado de Bloque III, clasificado como rate limiting real, no defecto de producto). Incremento 3 ("Perfil competitivo de otro usuario"): diseño completo (§11), decisiones de fondo confirmadas por el Product Owner (2026-08-06), y **`docs/adr/0021-perfil-competitivo-cross-cuenta.md` redactado -- PROPUESTA, pendiente de aprobación formal antes de implementar**. Pendiente: Incrementos 4-5 (pregunta rápida, superficie móvil).
...
**Resuelto (2026-08-06)**: el Product Owner confirmó las tres decisiones de fondo (redactar, nunca omitir — §11.3.2; lista blanca reconciliada — §11.3.1; resolución de identidad en la capa de presentación de USER, sin ciclo de módulos — §11.3.3), con una precisión adicional no anticipada por esta propuesta: la redacción también reemplaza el 404 uniforme de perfil inexistente/no presentable (Gate 5, §5 de este documento) por una respuesta 200 redactada, indistinguible entre "no presentable" e "inexistente" — privacidad estrictamente más fuerte que el diseño original. **`docs/adr/0021-perfil-competitivo-cross-cuenta.md` redactado** con estas decisiones formalizadas, 12 Decision Gates propuestos (sustituyen y amplían los de §11.6) — **PROPUESTA, pendiente de aprobación formal antes de tocar código**.
