...
**Estado**: **CERRADO — ver `docs/adr/LEF-BLOCK-IV-CLOSURE-REPORT.md`** (2026-08-07). Incremento 1 ("Fundación de temporadas y ligas") **implementado y gateado** (§9). Incremento 2 ("Ranking") **implementado, gateado y CERRADO** (§10, ADR-0020 APPROVED) — checkpoint commiteado sin tag (`26c28f1`). La limitación ambiental de `findPendingGrant` (Bloque I) documentada al cierre de Incremento 2 (§10, nota final) fue diagnosticada y corregida en un commit independiente (`0b52122`, starvation real confirmada y resuelta) — ver también `449d654`/`15fcd30` (hallazgo relacionado de Bloque III, clasificado como rate limiting real, no defecto de producto). Incremento 3 ("Perfil competitivo de otro usuario") **implementado, gateado y CERRADO** (§11, ADR-0021 APPROVED) -- sub-incrementos 3.a/3.b/3.c completos, checkpoints commiteados sin tag (`4eb676b`, `f7909da`, `7decfaa`). Incremento 4 ("Pregunta rápida") **implementado, gateado y CERRADO** (§12-13) -- entidades propias `quick_question_session`/`quick_question_attempt` (no el framework genérico del Data Model). Sub-incrementos 4.a (fundación de persistencia), 4.b (motor de sesión) y 4.c (endpoints HTTP y contratos) completos, checkpoints commiteados sin tag (`5ec321f`, `d1daafb`, `599c7b0`). Durante la regresión de 4.c se encontró y corrigió una fragilidad preexistente y no relacionada en `verify-reward-evaluation-worker-gate.ts` (Bloque III, sub-incremento 1.b) -- commit independiente sin tag (`7864dbe`), sin reabrir ni modificar el alcance de Bloque III ya cerrado. Incremento 5 ("Superficie móvil") **implementado, gateado y CERRADO** (§14) -- sub-incrementos 5.a (enrolamiento real de liga + hub), 5.b (ranking móvil), 5.c (perfil competitivo móvil) y 5.d (Pregunta rápida móvil) completos, checkpoints commiteados sin tag (`0c60194`, `54c5d52`, `61ea8d9`, `5f13394`). Verificación manual completa en Browser pane (hub, ranking, perfiles, Pregunta rápida) y en dispositivo Android físico, ambas en PASS — ver §14 para el detalle y los hallazgos investigados.
...
**Resuelto (2026-08-06) — Incremento 4 CERRADO.** Los tres sub-incrementos se implementaron y gatearon exactamente como se describe arriba, con dos correcciones encontradas y aplicadas durante la implementación (documentadas en sus respectivos commits, no en este diseño previo): (1) la publicación del evento (§13.3, punto 4) se corrigió para ser best-effort POST-COMMIT, nunca dentro de la transacción de escritura -- `OutboxService` usa el cliente Prisma global, conforme a ADR-0006; (2) se añadió una precisión no prevista aquí -- una sola `quick_question_session` `ACTIVE` por cuenta (índice único parcial + apertura idempotente bajo advisory lock), confirmada por el Product Owner al aprobar el brief de 4.b. Checkpoints commiteados sin tag: `5ec321f` (4.a), `d1daafb` (4.b), `599c7b0` (4.c). Durante la regresión de 4.c se encontró y corrigió, en un commit independiente sin tag (`7864dbe`), una fragilidad preexistente y no relacionada en el gate de `RewardEvaluationWorker` (Bloque III, sub-incremento 1.b, ya cerrado) -- sin reabrir ni modificar el alcance de Bloque III.

## 14. Incremento 5 — Superficie móvil: diseño confirmado, implementación y cierre

Reemplaza los tres shells "Próximamente" del tab Competir (Liga, ranking, Pregunta rápida) con las capacidades reales de los Incrementos 1-4, sobre `apps/mobile/app/(tabs)/competir/`, convertida de archivo plano a carpeta con `_layout.tsx` (Stack navigator) + `index.tsx` (hub), `ranking.tsx`, `perfil/[username].tsx`, `quick-question.tsx`.

### 14.1 Enrolamiento real (5.a) — corrección de fondo sobre el diseño inicial

El diseño original de este bloque (§8) dejaba el enrolamiento a una liga como deuda aceptada (`joinActiveSeason()` conectado implícitamente a `XpGrantService`). **Corregido por el Product Owner (2026-08-06) antes de implementar**: enrolamiento explícito e idempotente de autoservicio -- `GET/POST /gamification/me/league/participation`, unión discriminada (`ENROLLED`/`NOT_ENROLLED`/`NO_ACTIVE_SEASON`) para el GET, (`ENROLLED`/`NO_ACTIVE_SEASON`) para el POST, ambos `200` uniforme (nunca `201`, aunque la primera vez sí cree la fila). El GET nunca crea participación; el hub solo llama al POST desde el botón explícito "Unirme a la liga", nunca automáticamente al montarse. `LeagueEnrollmentService.getParticipationStatus`/`describeParticipation` son de solo lectura -- `joinActiveSeason()` permanece sin cambios, ahora invocado únicamente desde el POST de autoservicio.

### 14.2 Ranking móvil (5.b)

"Tu posición" (propia, siempre visible arriba) + lista paginada con botón "Ver más" (V1, no scroll infinito) -- reglas: `nextCursor` decide la existencia del botón (nunca el cliente decide cuándo dejar de pedir), un fallo de "Ver más" nunca reemplaza la lista ya cargada (mensaje aislado en el pie, `ErrorState` de pantalla completa reservado exclusivamente para el fallo de la carga inicial), el cliente nunca reinterpreta la redacción de filas no presentables (`presentable: false` ya expone únicamente `{presentable, isCurrentUser, rankPosition, metricValue}` por unión discriminada de TypeScript, imposible acceder a otros campos en tiempo de compilación). Terminología "puntos de liga" en toda la UI, nunca "XP".

### 14.3 Perfil competitivo móvil (5.c)

Componentes visuales compartidos (`identity-header`, `position-card`, `cosmetics-row`, `achievements-list`) reutilizados entre el perfil propio (sección embebida con estado/retry propios en `perfil.tsx`) y el de terceros (`perfil/[username].tsx`, solo lectura, alcanzable únicamente desde una fila presentable del ranking). Mismo 404 uniforme para `PRIVATE` y para un username inexistente -- nunca distingue. `username` recibido del backend sin normalización local, codificado como segmento de URL. `competitiveContext: null` reutiliza el mismo componente con copy contextual distinto según la variante (propio/tercero) vía `describePositionCardEmptyState`.

### 14.4 Pregunta rápida móvil (5.d)

Decisión de fondo confirmada por el Product Owner (2026-08-06): navegar hacia atrás **nunca** cierra la sesión -- una `QuickQuestionSession` `ACTIVE` queda simplemente abierta para retomarse después; solo el botón explícito "Salir" invoca `close()` (verificado estáticamente: única invocación de `closeQuickQuestionSession()` en todo el archivo, dentro de `handleExit`, nunca en un `useEffect` de desmontaje). Reabrir la pantalla puede devolver la pregunta pendiente de una sesión reutilizada -- la UI trata "nueva" y "reanudada" exactamente igual (`mapNextResult` no expone `isNew`/`isResumed`). Un nuevo intento genera un `operationId` nuevo; un retry de red ambiguo sobre la MISMA alternativa reutiliza exactamente el mismo `operationId` (`resolveAnswerOperationId`). Ante `409` en `/answers`, reconciliación con `/next` resuelve a exactamente uno de tres resultados, mutuamente excluyentes: pregunta disponible (adopta `question_presented`), sin preguntas (`no_questions`, informativo) o sesión cerrada (`session_closed`, terminal). Doble-toque bloqueado en el envío de respuesta y en "Siguiente pregunta" mediante los flags `submitting`/`loadingNext`, verificado tanto por lógica pura como por tres intentos reales concurrentes contra el servidor (una sola fila en base de datos).

### 14.5 Verificación manual (Browser pane + Android físico)

Recorrido funcional completo ejecutado en Browser pane (hub, ranking, perfiles, Pregunta rápida) el 2026-08-06/07, con dos verificaciones adicionales exigidas por el Product Owner: (1) cambio de tema durante la carga en curso de la segunda página del ranking -- confirmado mediante inspección de React Fiber que la lista permanece consistente (39 filas, `rankPosition` únicos y secuenciales de 1 a 39, sin duplicados ni huecos); (2) cierre completo de la aplicación durante una sesión activa de Pregunta rápida -- **no reproducible literalmente en el target Web** porque `expo-secure-store` no persiste sesión entre recargas completas del navegador (confirmado empíricamente); sustituido por la prueba equivalente más cercana alcanzable en Browser pane (desmontaje/remontaje real de la pantalla vía navegación dentro del mismo Stack), que sí recuperó la misma pregunta pendiente -- la confirmación literal quedó pendiente de Android físico.

**Hallazgo investigado y descartado durante el recorrido**: el ranking parecía "atascarse" en 10 filas visibles tras "Ver más". Descartado como defecto de producto mediante tres verificaciones independientes (fetch directo al backend, conteo en base de datos, inspección de `FlatList.memoizedProps.data.length`) -- es una limitación de virtualización (`initialNumToRender`) de React Native Web en un navegador headless sin scroll táctil real, no un error de paginación/merge/dedup. No se modificó código de producto.

**Hallazgo de diseño confirmado durante la limpieza de datos de verificación**: `league_point_ledger_entry` es **append-only por diseño** -- un trigger de base de datos (`enforce_league_point_ledger_entry_no_delete`) rechaza cualquier `DELETE`, toda corrección debe ser una entrada compensatoria (§9.5, §9.8). Esto explica y **corrige** una sospecha inicial de "higiene incompleta" en un gate anterior (`verify-competitive-leaderboard-gate.ts`, que dejó una temporada/grupo de prueba sin limpiar): no es un bug de higiene del gate, es consecuencia esperada de la inmutabilidad del ledger -- ningún gate puede borrar entradas ya escritas. Como consecuencia, los datos sintéticos creados por el script temporal de siembra de esta verificación manual (25 participantes, 2 `league_point_rule`, 3 `public_profile`, nunca commiteado, ya eliminado) quedan como residuo permanente e inofensivo en la base de datos de desarrollo -- no se intentó ningún workaround de borrado forzado.

**Verificación en dispositivo Android físico** (2026-08-07, Product Owner): recorrido completo repetido en Android real -- gestos del sistema, TalkBack, doble toque táctil real, contraste visual percibido, comportamiento en Expo Go, y la confirmación literal de cierre completo de la aplicación durante una sesión activa de Pregunta rápida con recuperación de la misma pregunta pendiente al reabrir. **PASS, sin hallazgos.**

**Resuelto (2026-08-07) — Incremento 5 CERRADO.** Los cuatro sub-incrementos se implementaron y gatearon exactamente como se describe arriba. Checkpoints commiteados sin tag: `0c60194` (5.a), `54c5d52` (5.b), `61ea8d9` (5.c), `5f13394` (5.d). Verificación manual completa (Browser pane + Android físico) en PASS, cero defectos de producto encontrados -- ver §14.5. **Bloque IV — Competir: CERRADO.** Ver `docs/adr/LEF-BLOCK-IV-CLOSURE-REPORT.md`.
