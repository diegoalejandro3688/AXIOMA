...
| **C3.5A** | **Bridge IAP nativo + build Billing-enabled.** *(Puede avanzar sin cuenta Play — B-0 no lo bloquea.)* Módulo IAP (`expo-iap` recomendado) como dep **móvil**; `expo prebuild` según lo requiera + rebuild. **Registrar la versión real de `com.android.billingclient:billing`** (§B.1). Debug APK con Billing compilado y sideloadeado. `expo-dev-client` opcional. Sin lógica de compra todavía. **El primer upload a Play queda `DEFERRED` hasta que exista la cuenta Play (B-0).** | No | **SÍ** (módulo IAP + prebuild + rebuild) | `DEFERRED` (primer upload) | — | Sideload del build |
...
C3.5A   bridge IAP nativo + PBL resuelto + debug APK con Billing        [puede avanzar]
...
- **C3.5A** (base nativa de Billing: módulo IAP + `expo prebuild` + resolver la versión de PBL + debug APK con Billing) **puede proceder ya** — no depende de Play Console. Sólo el *primer upload a Play* dentro de C3.5A queda `DEFERRED`.
