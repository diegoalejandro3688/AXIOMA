...
| El wrapper (expo-iap / react-native-purchases) es quien fija la versión de PBL empaquetada. | La decisión práctica es *qué wrapper*, no *qué PBL* directamente. **Invariante de implementación (cumplido en C3.5A)**: tras instalar el bridge, **inspeccionar y registrar la versión de `com.android.billingclient:billing` realmente resuelta** en el proyecto Android generado, en vez de asumir la del wrapper. **Resultado C3.5A (`gradlew :app:dependencyInsight`, 2026-09-03):** `expo-iap 5.5.0 → io.github.hyochan.openiap:openiap-google 3.5.0 → com.android.billingclient:billing:9.1.0` — PBL **9.1.0**, la última soportada. Objetivo cumplido. |
...
2. `expo-iap` minimiza la fricción de prebuild/CNG (está diseñado para binarios nativos propios de Expo, `expo-dev-client` opcional) sin imponer un backend de terceros. **C3.5A (hecho):** `expo-iap@5.5.0` → `openiap-google 3.5.0` → PBL **9.1.0** (invariante de B.1 cumplido, evidencia Gradle real); autolinking sin config plugin; sin `expo-dev-client`.
...
| `linkedPurchaseToken` | String? | `SubscriptionPurchaseV2.linkedPurchaseToken` (rotación EN VIVO) | predecesor → `SUPERSEDED` — ver D.4.1 |
| `resubscribedFromPurchaseToken` | String? | `SubscriptionPurchaseV2.outOfAppPurchaseContext.expiredPurchaseToken` | re-alta fuera de la app tras expiración total — SOLO atribución, el predecesor NO se marca `SUPERSEDED` — ver D.4.4 |
...
| `startTime` | DateTime? | `SubscriptionPurchaseV2.startTime` | inicio de la suscripción — **generación de compra**, criterio (1) de "fila vigente" (§D.3) |
...
- **Selección de la fila "vigente"** (`findCurrentByAccountId`, C3.3 seguimiento resubscribe): `where state != SUPERSEDED`, `orderBy` = (1) **`startTime` DESC NULLS LAST** — *generación de compra*: una re-alta (in-app, fuera de la app, o token rotado) es una compra NUEVA con `startTime` (dato autoritativo de Google) más nuevo que la expirada; una renovación NO cambia `startTime`. (2) `latestEventTime` DESC NULLS LAST — cronología de proveedor dentro de la misma generación. (3) `updatedAt` DESC → `id` DESC — desempate local determinista. Consecuencia: una compra NUEVA verificada (`latestEventTime` aún `null`) es la vigente aunque una fila histórica tenga un `eventTimeMillis` conocido → PREMIUM inmediato tras `reconcile`, sin esperar el RTDN `SUBSCRIPTION_PURCHASED`. Y una fila ACTIVE stale (misma generación o más vieja) nunca gana a una verdad de proveedor más nueva REVOKED/EXPIRED.
...
4. **`outOfAppPurchaseContext.expiredPurchaseToken`** — RE-ALTA FUERA DE LA APP tras EXPIRACIÓN TOTAL (C3.3 seguimiento). Semántica **DISTINTA** a `linkedPurchaseToken`: la suscripción anterior **ya terminó** (`EXPIRED`) — esto es una compra **nueva**, no un reemplazo en vivo. Reglas: (a) **NO** se marca la fila anterior `SUPERSEDED` — se queda `EXPIRED` (su estado terminal natural); la fila nueva la supera por `startTime` (§D.3). (b) `expiredPurchaseToken` sólo sirve para **ATRIBUIR la cuenta** cuando el RTDN `SUBSCRIPTION_PURCHASED` llega antes de que el móvil reconcilie: `findByPurchaseToken(expiredPurchaseToken)` → `accountId` heredado. (c) Se registra el vínculo en `resubscribed_from_purchase_token` (columna nueva, aditiva; **no** es `linkedPurchaseToken`). (d) **Ownership**: si la fila expirada pertenece a otra cuenta que la de la sesión → cadena imposible → `SUBSCRIPTION_ACCOUNT_MISMATCH`, sin transferir. (e) Si `expiredPurchaseToken` no existe como fila → no se puede atribuir por aquí; el worker reintenta acotadamente (24 → `FAILED`), nunca fabrica una fila. (f) `expiredExternalAccountIdentifiers.obfuscatedExternalAccountId` se mapea como diagnóstico — **fallback futuro** de atribución (C3.5 fijará un `obfuscatedAccountId` opaco en el BillingFlow); hoy `expiredPurchaseToken` basta para el caso primario.
5. **`SUBSCRIPTION_STATE_PENDING_PURCHASE_CANCELED`** (compra pendiente cancelada — C3.2 hardening): el `linkedPurchaseToken` NO indica un reemplazo completado, sino uno que **se canceló antes de completarse**. El token cancelado **nunca es una fila** (`AccountSubscription`), **nunca acknowledgea** y **nunca marca `SUPERSEDED`** a la suscripción linkeada. Con `linkedPurchaseToken` presente se reconsulta y reconcilia esa suscripción existente (por sus efectos — el entitlement se deriva de ELLA); sin él, resultado neto = nada. Si la suscripción linkeada pertenece a otra cuenta → `SUBSCRIPTION_ACCOUNT_MISMATCH`, sin transferir. **El `status` del endpoint describe la disposición del token que el cliente envió: SIEMPRE `canceled` para un token en este estado**, aunque el entitlement resultante (vía `GET /me/entitlement`) sea PREMIUM por la suscripción previa. Devolver `verified` haría que el móvil interpretara un intento de compra cancelado como exitoso.
...
| **Re-alta fuera de la app antes de expirar** (Play Store → "Resubscribe" desde suscripciones canceladas, aún dentro del periodo) | RTDN (`SUBSCRIPTION_RESTARTED`, token nuevo con `linkedPurchaseToken`) → predecesor `SUPERSEDED` → el backend reconcilia sin intervención del cliente (§D.4.1). |
| **Re-alta fuera de la app TRAS EXPIRACIÓN TOTAL** (Play Store → resubscribirse a una suscripción ya expirada) | RTDN (`SUBSCRIPTION_PURCHASED`, token nuevo **sin** `linkedPurchaseToken`; `SubscriptionPurchaseV2` trae `outOfAppPurchaseContext.expiredPurchaseToken`). El worker resuelve la cuenta desde la fila expirada, reconcilia y acknowledgea la compra nueva **sin que la app se abra** (§D.4.4). La fila expirada NO se marca `SUPERSEDED`. |
...
Tabla oficial ACTUAL de `subscriptionNotification.notificationType` (revisada 2026-09): 1 `SUBSCRIPTION_RECOVERED`, 2 `SUBSCRIPTION_RENEWED`, 3 `SUBSCRIPTION_CANCELED`, 4 `SUBSCRIPTION_PURCHASED`, 5 `SUBSCRIPTION_ON_HOLD`, 6 `SUBSCRIPTION_IN_GRACE_PERIOD`, 7 `SUBSCRIPTION_RESTARTED`, 8 `SUBSCRIPTION_PRICE_CHANGE_CONFIRMED` **(deprecado)**, 9 `SUBSCRIPTION_DEFERRED`, 10 `SUBSCRIPTION_PAUSED`, 11 `SUBSCRIPTION_PAUSE_SCHEDULE_CHANGED`, 12 `SUBSCRIPTION_REVOKED`, 13 `SUBSCRIPTION_EXPIRED`, 17 `SUBSCRIPTION_ITEMS_CHANGED`, 18 `SUBSCRIPTION_CANCELLATION_SCHEDULED`, 19 `SUBSCRIPTION_PRICE_CHANGE_UPDATED`, 20 `SUBSCRIPTION_PENDING_PURCHASE_CANCELED`, 22 `SUBSCRIPTION_PRICE_STEP_UP_CONSENT_UPDATED`. (14–16, 21 y todo entero futuro no documentado → `SUBSCRIPTION_UNKNOWN_<n>`.) También `oneTimeProductNotification` (no aplica — no hay one-time products) y `testNotification` (log + ACK, sin efecto). **Manejo uniforme**: cualquier `subscriptionNotification` con `purchaseToken` → reconsultar el token → re-derivar. El `notificationType` sólo se registra (`latestNotificationType`) y sirve para métricas/alertas — **ningún tipo concede ni revoca entitlement por sí mismo**. **Reconocer ≠ soportar**: 17/18/19/22 se reconocen y registran pero NO implican add-ons, cuotas ni UI de precio en V1 (un `ITEMS_CHANGED` que añadiera un add-on lo rechaza el mapper C3.2 — line item ≠ `premium-monthly`).
...
### H.6 C3.3 — implementación (commit `feat(premium): add google play rtdn processing`)

Refina §H.1–H.4 con lo realmente construido (backend, sin Pub/Sub real):

- **Ruta**: `POST /internal/google-play/rtdn` (convención `internal/` del repo, no `/_google/...`). Sin `AuthGuard` de usuario ni `InternalOpsGuard` — frontera OIDC propia (`RtdnPushAuthenticator`). El disparo manual del worker es `POST /internal/google-play/rtdn/_internal/process` (`InternalOpsGuard`, patrón `privacy/_internal/sweep`).
- **OIDC**: `google-auth-library` `OAuth2Client.verifyIdToken` (real, certs públicos de Google, sin credenciales propias) o fake (gates). Valida firma + `aud` (`GOOGLE_PLAY_RTDN_OIDC_AUDIENCE`) + `email` == service account de push (`GOOGLE_PLAY_RTDN_PUSH_SERVICE_ACCOUNT_EMAIL`) + `email_verified === true` + expiración. Factory `resolveRtdnAuthChoice`: en producción sin `GOOGLE_PLAY_RTDN_AUTH_IMPL=google` o con config incompleta → **LANZA al arrancar** (fail-closed, mismo criterio que `resolveSubscriptionProviderChoice` C3.2). Rechazo → **401** (no 403). El bearer nunca se loguea ni se persiste.
- **Buzón durable** (`google_play_rtdn_event`, migración aditiva): la entrega de Pub/Sub se **persiste y deduplica ANTES** de responder 2xx — `subscriptionsv2.get` NO bloquea el ACK a Pub/Sub. Un fallo de Google/red tras la entrega no pierde el evento. Dedup por `messageId` (`@unique`, sobrevive reinicios — no un `Set` en memoria). `test`/one-time/voided/familia desconocida se registran ya resueltos (`DONE`/`IGNORED`), nunca disparan reconciliación ni entitlement.
- **Worker** (`@Cron` `EVERY_MINUTE` + trigger manual): claim atómico (`updateMany` condicionado por `status`+`updatedAt`, excluye ids ya tratados en la pasada, recupera `PROCESSING` > 5 min). Reusa **la misma** reconciliación C3.2 vía `reconcileFromNotification({ purchaseToken, providerEventTime, notificationType })` — sin segundo mapper ni segunda ruta de persistencia. Cuenta: de la fila existente por `purchaseToken`, o heredada del predecesor `linkedPurchaseToken` (ADR D.4.1); si no se puede atribuir todavía (RTDN antes del reconcile del móvil) → `RETRYABLE` acotado (24 intentos → `FAILED`), nunca loop. Errores: transitorio/ack/no-atribuible → `RETRYABLE`; package/token inválido confirmado / mismatch → `FAILED` permanente.
- **`latestEventTime` autoritativo + MONÓTONO** (`resolveProviderEventTimeUpdate`, puro): `DeveloperNotification.eventTimeMillis` es la única fuente; nunca llegada/`updatedAt`/`Date.now()`. `incoming ≤ existing` (RTDN atrasado / fuera de orden) → **no retrocede** y no pisa `latestNotificationType`. El reconcile directo del móvil pasa `providerEventTime = null` → jamás fija ni borra la cronología (`createFromVerified`/`updateFromVerified` intactos; columna sólo vía `applyProviderEvent`).
- **Revocación** (`applyRevocationContext`, puro): `SubscriptionPurchaseV2` no expone "revoked"; si el RTDN es `SUBSCRIPTION_REVOKED` (12) **y** el snapshot reconsultado ya es `EXPIRED`, se registra `state = REVOKED` — sólo fidelidad de auditoría (ambos derivan `FREE`). Si el snapshot dijera un estado que concede, no se sobreescribe (se confía en Google, se loguea posible desincronización).
- **`SUBSCRIPTION_PENDING_PURCHASE_CANCELED`** (tipo 20) fluye por el camino C3.2 ya aprobado (disposición del mapper; B nunca es fila, A no `SUPERSEDED`, entitlement de A) — sin implementación especial duplicada.
- **Re-alta FUERA de la app tras expiración total** (`outOfAppPurchaseContext.expiredPurchaseToken`, transporte + mapper + snapshot neutral): el worker resuelve la cuenta desde la fila expirada, reconcilia y acknowledgea B **sin abrir el móvil**; la fila expirada NO se marca `SUPERSEDED` (se registra el vínculo en `resubscribed_from_purchase_token`, columna aditiva). Ownership: fila expirada de otra cuenta → `SUBSCRIPTION_ACCOUNT_MISMATCH`; sin fila → reintento acotado, nunca fila fabricada. Ver §D.4.4. La selección de "fila vigente" ahora antepone `startTime` (generación de compra) para que la re-alta sea PREMIUM inmediato tras el `reconcile` in-app, sin esperar el RTDN (§D.3).
- **Tipos de notificación** (`rtdn-notification-type.ts`): mapa explícito con TODA la tabla oficial actual (1–13, 17, 18, 19, 20, 22; el 8 marcado deprecado en `RTDN_SUBSCRIPTION_DEPRECATED_TYPES`). 17/18/19/22 (`ITEMS_CHANGED` / `CANCELLATION_SCHEDULED` / `PRICE_CHANGE_UPDATED` / `PRICE_STEP_UP_CONSENT_UPDATED`) se **reconocen** (nunca `SUBSCRIPTION_UNKNOWN_*`) y disparan la reconsulta autoritativa como cualquier evento con `purchaseToken` — pero *reconocer ≠ soportar*: V1 NO implementa add-ons / cuotas / UI de cambio de precio. `notificationType` no documentado (14–16, 21, futuros) → `SUBSCRIPTION_UNKNOWN_<n>`, se registra, dispara la reconsulta igual, nunca concede desde el RTDN.
- **Deferido** a C3.4+: crear el topic + push subscription + service account reales; probar con `subscriptionsv2.get` real; `SubscriptionEvent` append-only para forense (Q-6) si se decide.
...
| **C3.2** ✅ | **Adaptador Google real (server-side).** `google-auth-library` (dep de **backend**, no móvil). `GooglePlaySubscriptionAdapter` → `subscriptionsv2.get` + `acknowledge`. Config: `GOOGLE_PLAY_PROVIDER_IMPL`, `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` (el `packageName` es la constante `com.zetrynd.app`, no un env). Gate contra el fake. Todavía sin cliente móvil. | No | No | **Service account + acceso lectura Play** (setup en C3.4A) | No | No |
| **C3.3** ✅ | **RTDN backend.** `POST /internal/google-play/rtdn` + verificación OIDC (fail-closed en prod) + buzón durable `google_play_rtdn_event` (dedup por `messageId`, persist-antes-de-ACK) + worker `@Cron` que reusa `reconcileFromNotification` (extensión mínima de C3.2 con `providerEventTime`). `latestEventTime` monótono. `verify:google-play-rtdn-gate`. Ver §H.6. | **SÍ** (`google_play_rtdn_event`, aditiva) | No | — | Diferido a C3.4 (topic + push subscription reales) | No |
| **C3.4A** — *doc/readiness* | **Readiness de infraestructura Google Cloud / control-plane (SIN crear productos).** Runbook `docs/adr/PREMIUM-V1-LAYER-3-GOOGLE-SETUP-RUNBOOK.md`: auditoría, nombres/IDs/emails de recursos, permisos mínimos, config exacta pendiente de la push subscription (falta HTTPS público), pasos de operador para C3.4B, blockers. **Restricción confirmada 2026-09-03: el PO NO tiene cuenta de Google Play Console** → todo lo dependiente de Play Console = `DEFERRED — PLAY CONSOLE NOT AVAILABLE`; Google Cloud/Pub/Sub = `PLANNED` (opcional, no bloquea). **NO** se crea ningún recurso externo en este increment. `.env.example` apunta al runbook. Sin cambios de código de producto. | No | No | `DEFERRED — PLAY CONSOLE NOT AVAILABLE` | `PLANNED` (topic + IAM, opcional; push subscription PENDIENTE por HTTPS) | — |
| **C3.5A** ✅ **DONE** | **Bridge IAP nativo + build Billing-enabled.** `expo-iap@5.5.0` (fijado exacto) como dep **móvil**; `expo prebuild -p android --clean`; autolinking reconoce `ExpoIapModule` **sin config plugin**. **PBL real resuelta = `com.android.billingclient:billing:9.1.0`** (`gradlew :app:dependencyInsight` externo en la máquina del PO → `expo-iap 5.5.0 → openiap-google 3.5.0 → billing 9.1.0`; §B.1 cumplido). `assembleDebug` **BUILD SUCCESSFUL**, APK instalado en el Samsung `R5CW71R7MTP`, smoke físico PASS (login/Inicio/Estudio/paywall/Competir/Tutor IA/Perfil/reopen, sin crash, sin "ExpoIapModule not found"). Probe estructural de `bundleRelease` → AAB producido (56 MB). Sin lógica de compra. `expo-dev-client` **no** añadido. `verify:premium-native-billing-foundation-gate`. Ver `docs/adr/PREMIUM-V1-LAYER-3-NATIVE-BILLING-FOUNDATION-CLOSURE-REPORT.md`. **El primer upload a Play sigue `DEFERRED` (B-0).** | No | **SÍ** (`expo-iap@5.5.0` + prebuild + build) — evidencia externa Windows/Gradle | `DEFERRED` (primer upload — B-0) | — | ✅ smoke físico PASS (sin compra) |
| **C3.4B** *(bloqueado por B-0)* | **Crear/activar el producto en Play Console.** Requiere: cuenta Play creada + app `com.zetrynd.app` + build Billing-enabled subida. Crear `zetrynd_premium` + base plan `premium-monthly` + precio Chile `$6.990 CLP/mes` (sin offer V1). Verificar `queryProductDetails` real. Configurar License Testers. Poner el backend en `GOOGLE_PLAY_PROVIDER_IMPL=google` + secret. **IDs efectivamente permanentes** una vez activados. | No | No | **SÍ** (crear subscription + base plan + precio + license testers) | — | — |
| **C3.5B** *(QA real bloqueado por B-0)* | **Integración de compra/restore + QA físico de Billing.** `lib/billing/`, `lib/api/subscription.ts`, `useBilling`, `startPremiumPurchase`, `restorePurchases`. Gates mobile puros de `BillingState`/flujo (sin red — pueden avanzar). Paywall **todavía** con la constante temporal. El **QA real** en el Samsung `R5CW71R7MTP` (`ProductDetails`, compra de prueba, verify server-side, reconcile) necesita producto + License Testers → `DEFERRED` hasta la cuenta Play (C3.4B). | No | rebuild | — | — | `DEFERRED` (necesita C3.4B) |
| **C3.4-KS** *(en paralelo; obligatorio antes de C3.8)* | **Keystore de producción + Internal Track.** Resolver la deuda de `signingConfigs.debug` (Q-3): keystore de producción (o Play App Signing + upload key). Build de **release firmada** en *Internal testing*. **No** bloquea el primer QA de Billing. | No | **Build de release firmada** | **SÍ** (Internal testing) | — | Instalación desde Internal Track |
...
| **C3.8** | **QA de ciclo de vida + Internal Track + cierre.** Play Billing Lab: grace / hold / recovery / revoke / renewal / cancel-still-entitled (sobre debug APK + License Tester). **QA de distribución realista desde el Internal Track (C3.4-KS): descarga desde Play, updates, firma real.** ADR de cierre `PREMIUM-V1-LAYER-3-BILLING-CLOSURE-REPORT.md`. | No | — | Play Billing Lab + Internal Track | — | **SÍ** — batería completa §M.3 + Internal Track |

### N.1 Corrección de secuenciamiento (C3.4 → C3.4A / C3.5A / C3.4B / C3.5B)

Google exige una **build con Billing subida a Play** antes de que la configuración de productos de Billing quede plenamente habilitada, y los IDs de producto/base plan son **efectivamente permanentes** una vez creados. Por eso el antiguo "C3.4 — crear producto" se parte:

```
C3.4A   runbook de readiness (SIN crear recursos; Play Console DEFERRED por B-0)          ← este increment
C3.5A   bridge IAP nativo + PBL 9.1.0 resuelto + debug APK + smoke Samsung   [✅ DONE 2026-09-03]
         └─ primer upload a Play                                        [DEFERRED por B-0]
C3.4B   crear/activar zetrynd_premium + premium-monthly + $6.990 CLP + License Testers   [DEFERRED por B-0]
C3.5B   integración compra/restore (gates puros: avanzan) + QA real en el Samsung        [QA real DEFERRED por B-0]
```

`C3.4-KS` (keystore de producción + Internal Track) corre en paralelo; **el Internal Track sigue siendo obligatorio antes del cierre de la Capa 3 (C3.8)**, no antes del primer QA de Billing.

**Restricción 2026-09-03 — el PO no tiene cuenta de Google Play Console** (blocker **B-0** del runbook). Efecto en la secuencia:

- **C3.4A** entrega sólo el runbook. Recursos Play Console → `DEFERRED`; recursos Google Cloud/Pub/Sub → `PLANNED` (opcionales, no bloquean el cierre de C3.4A).
- **C3.5A** ✅ **DONE** (2026-09-03): `expo-iap@5.5.0` fijado; PBL real resuelta **9.1.0** (evidencia externa Gradle); `assembleDebug` OK; smoke físico Samsung PASS (sin compra); probe estructural de AAB OK. Cierre en `PREMIUM-V1-LAYER-3-NATIVE-BILLING-FOUNDATION-CLOSURE-REPORT.md`. Sólo el *primer upload a Play* queda `DEFERRED` (B-0).
- **C3.4B** y el QA real de Billing (**C3.5B**) quedan `DEFERRED` hasta que exista la cuenta Play (USD 25 + verificación de identidad + perfil de pagos con Chile). El PO aporta identidad / datos fiscales / bancarios — el agente no.

Hitos:

- **Migración Prisma**: C3.1, C3.3 (`google_play_rtdn_event`), C3.3-seguimiento (`resubscribed_from_purchase_token`).
- **Dependencia nativa / rebuild**: C3.5A (móvil). `expo-dev-client` **opcional**. El backend (C3.2/C3.3) no toca nativo.
- **Runbook de infraestructura Google**: **C3.4A** — `PREMIUM-V1-LAYER-3-GOOGLE-SETUP-RUNBOOK.md` (entregado). Recursos Google Cloud/Pub/Sub = `PLANNED` (opcionales); recursos Play Console = `DEFERRED — PLAY CONSOLE NOT AVAILABLE` (B-0).
- **Cuenta de Google Play Console**: **NO existe** (B-0). El PO debe crearla (USD 25 + verificación de identidad + perfil de pagos con Chile). Bloquea C3.4B, el primer upload de C3.5A y el QA real de C3.5B.
- **Play Console — producto + License Testers**: **C3.4B** (tras B-0 + el primer upload Billing-enabled de C3.5A).
- **Keystore de producción + Internal Track**: **C3.4-KS**, en paralelo; **obligatorio antes del cierre (C3.8)**, no antes del primer QA de Billing.
- **Samsung físico con Billing real**: **C3.5B**, `DEFERRED` hasta C3.4B. Batería completa de ciclo de vida en C3.8.
