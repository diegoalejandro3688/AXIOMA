...
- Como ZETRYND **ya compila su propio binario nativo** (`expo run:android` / `gradlew assembleDebug`), añadir una dependencia IAP **no obliga a un cambio de paradigma**: es exactamente la clase de módulo nativo que un CNG build ya soporta. Lo que cambia:
  ...
  4. Google Play Billing **sólo transacciona con una app instalada desde Google Play** (o desde un track de testing interno / License Testing). El debug APK sideloadeado **no puede** completar una compra real; puede inicializar `BillingClient` y consultar `queryProductDetails` sólo si el `applicationId` está publicado y la cuenta de Google del dispositivo es *license tester*. Ver §M.
- **Transición mínima recomendada a un "development build" de ZETRYND** (a ejecutar en C3.x, **no** en C3.0): añadir `expo-dev-client` para tener el dev-menu + carga de Metro sobre un binario con módulos nativos, y **crear un `flavor`/`applicationId` de desarrollo distinto** sólo si se quiere instalar dev y prod en paralelo (opcional — hoy ambos son `com.zetrynd.app`). El workflow Metro + Samsung físico **se conserva**: `expo start` + dev build instalado. La única diferencia real frente a hoy es `expo-dev-client` (dev-menu) y que las builds de QA de Billing deben venir de un track interno de Play o License Testing.

**Conclusión A**: no hay un salto de arquitectura de runtime; hay un salto de **procedencia del binario** (Play track / License Testing en vez de sideload) y un **rebuild nativo** al integrar el módulo. El sandbox del agente **no puede** compilar Android — todo build nativo lo ejecuta el Product Owner.
...
| El wrapper (expo-iap / react-native-purchases) es quien fija la versión de PBL empaquetada. | La decisión práctica es *qué wrapper*, no *qué PBL* directamente — pero hay que **verificar** que la versión del wrapper elegido empaqueta PBL ≥ 8 en el momento de integrar. |
...
| Compatibilidad Expo / CNG | Requiere manejar autolinking + (a veces) parches; menos "Expo-first". | **Diseñado para dev builds de Expo**, con config plugin. Menor fricción de prebuild. | Config plugin oficial, compatible con CNG. Requiere dev build. |
...
| Testabilidad | License Testing + track interno. Fakes propios en backend. | Igual. | RevenueCat añade su propio sandbox/dashboard, pero también su propia superficie a mockear. |
...
2. `expo-iap` minimiza la fricción de prebuild/CNG (está diseñado para dev builds de Expo) sin imponer un backend de terceros. Empaqueta Play Billing moderno; hay que verificar en el momento de integrar que la versión traiga **PBL ≥ 8**.
...
| 4 | **Google Play License Testing** | Compras reales de prueba sin cargo, con cuentas *license tester*, sobre un APK con el `applicationId` publicado (track interno). Prueba `ProductDetails` reales, `launchBillingFlow`, `purchaseToken` reales, acknowledge, `subscriptionsv2.get` real. | **Samsung físico**, APK de track interno de Play. |
| 5 | **Play Billing Lab** | Simular renovaciones aceleradas, grace period, account hold, recovery, revoke — estados que license testing solo no cubre bien. | Play Console + dispositivo. |
| 6 | **Track interno de Play** | Build firmada (keystore de producción — pendiente §Q-3) subida a *Internal testing*; instalada por testers reales. Es el primer punto donde Billing transacciona de verdad. | Google Play. |
...
### M.2 ¿Qué NO se puede testear en el runtime actual?

- **Nada de Billing real** con el debug APK sideloadeado actual: `BillingClient.launchBillingFlow` requiere que la app esté instalada desde Google Play (o un track de testing) y que la cuenta de Google sea *license tester*. → Capas 4–7 requieren la publicación en un track interno.
...
| **C3.4** | **Producto en Play Console.** Crear `zetrynd_premium` + base plan `premium-monthly` + precio CLP. **Keystore de producción** + firma real (resuelve la deuda de `signingConfigs.debug`). Primera subida a *Internal testing*. | No | **Build de release firmada** | **SÍ** (crear subscription + base plan + precio + license testers) | — | Instalación desde track interno |
| **C3.5** | **Cliente móvil de billing.** `expo-iap` (dep **móvil** + config plugin). `expo-dev-client`. `lib/billing/`, `lib/api/subscription.ts`. `useBilling`, `startPremiumPurchase`, `restorePurchases`. **`expo prebuild` + rebuild nativo.** Gates mobile puros de `BillingState`/flujo. Paywall **todavía** con la constante temporal. | No | **SÍ** (`expo-iap` + `expo-dev-client` + prebuild + rebuild) | — | — | **SÍ** — License Testing: `ProductDetails`, compra de prueba, verify |
| **C3.6** | **Paywall real.** Sustituir `Disponible próximamente` → `Suscribirme`; estados purchasing/pending/canceled/verified/error; precio localizado; eliminar `PREMIUM_PRICE_DISPLAY`. `verify:premium-paywall-gate` actualizado. | No | rebuild | — | — | **SÍ** — flujo completo end-to-end |
| **C3.7** | **Perfil → Suscripción.** Pantalla + `GET /me/subscription` consumido + deep link a Play. | No | rebuild | — | — | **SÍ** |
| **C3.8** | **QA de ciclo de vida + cierre.** Play Billing Lab: grace / hold / recovery / revoke / renewal / cancel-still-entitled. ADR de cierre `PREMIUM-V1-LAYER-3-BILLING-CLOSURE-REPORT.md`. | No | — | Play Billing Lab | — | **SÍ** — batería completa §M.3 |

Hitos:

- **Migración Prisma**: C3.1 (y posiblemente C3.3).
- **Dependencia nativa / dev build**: C3.5 (móvil). El backend (C3.2/C3.3) no toca nativo.
- **Play Console**: C3.4 (setup de service account en C3.2).
- **Google Cloud Pub/Sub**: C3.3.
- **Samsung físico con Billing real**: desde C3.5 (License Testing); batería completa en C3.8.
...
| Q-3 | **Keystore de producción y estrategia de firma** (hoy `release` usa la debug key). Bloqueante para cualquier track de Play. | Generar y custodiar un keystore de producción (o Play App Signing con upload key) antes de C3.4. |
