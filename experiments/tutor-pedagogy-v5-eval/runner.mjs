/**
 * Runner de la evaluación pedagógica de `AXIOMA_TUTOR_V5`.
 *
 * Copia del runner de V4 (`experiments/tutor-pedagogy-v4-eval/runner.mjs`), que queda
 * CONGELADO e intacto junto con su evidencia real. CERO cambios de mecánica: mismos
 ...
 * misma lectura de metadata del ledger. Lo único distinto es el directorio en el que vive
 * (y por tanto el `dataset/cases.json` que lee y el `results/` donde escribe) y la etiqueta
 * de los uid de sesión (`tutor-v5-eval-*`). No se reinventa ningún mecanismo.
 *
...
async function createSession(tag) {
  const uid = `tutor-v5-eval-${tag}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
