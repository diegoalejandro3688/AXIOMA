/**
 * Runner de la evaluación pedagógica de `AXIOMA_TUTOR_V6_1` (subset de reevaluación del
 * parche acotado sobre el bloque WORKED_SOLUTION).
 *
 * Copia del runner de V6 (`experiments/tutor-pedagogy-v6-eval/runner.mjs`), que queda
 * CONGELADO e intacto junto con su evidencia real. Diferencias, todas mínimas y explícitas:
 * (a) directorio/dataset/results de V6.1; (b) etiqueta de uid `tutor-v6_1-eval-*`;
 * (c) PRECONDICIÓN DE IDENTIDAD AMPLIADA: `assertBackendIdentity()` ya no se conforma con
 *     confirmar el provider efectivo -- exige además que el proceso que escucha tenga
 *     cargada EXACTAMENTE la versión de prompt esperada (`--expect-prompt-version`,
 *     por defecto `AXIOMA_TUTOR_V6_1`), leída de
 *     `GET /ai/_internal/effective-provider → promptVersion`, que el backend deriva de la
 *     constante `AXIOMA_TUTOR_PROMPT_VERSION` que tiene EN MEMORIA. Sin esto, un backend
 *     viejo (V6) todavía en pie tras editar el prompt produciría una corrida pagada que
 *     mide el prompt equivocado, y solo se detectaría A POSTERIORI leyendo el ledger.
 *
 ...
 *       `experiments/tutor-pedagogy-v6_1-eval/results/`;
 *   (2) etiqueta de uid de sesión `tutor-v6_1-eval-*`;
 ...
 */
const expectedPromptVersion = arg('expect-prompt-version', 'AXIOMA_TUTOR_V6_1');
...
async function assertBackendIdentity(expectedProvider) {
  ...
  const actualPromptVersion = probe.body?.promptVersion;
  if (actualPromptVersion !== expectedPromptVersion) {
    throw new Error(
      `VERSIÓN DE PROMPT EQUIVOCADA en ${base}: se esperaba promptVersion="${expectedPromptVersion}" y el proceso que escucha tiene cargada promptVersion="${actualPromptVersion}". ` +
        `Un provider correcto NO prueba que el TEXTO de instrucciones sea el que se quiere medir (backend viejo todavía en pie tras editar el prompt). Abortando ANTES de emitir la primera llamada de la matriz.`,
    );
  }
  console.log(`Versión de prompt CONFIRMADA en ${base}: promptVersion=${actualPromptVersion} -- esperado "${expectedPromptVersion}".`);
...
async function createSession(tag) {
  const uid = `tutor-v6_1-eval-${tag}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
